...
| 20 | **kb_ingest 重复执行** | `internal/agent/agent.go` | `preDispatchToolCall` 在流式期间后台执行 `kb_ingest`，若主循环 `takePreDispatchResult` 未及时返回，会再次执行。→ 加入 side-effect 工具黑名单（kb_ingest/write_to_file/replace_in_file/execute_command），跳过预分发。 |
...
### 待提交
8. **P2.5.3 主题系统组件集成** — 14 个文件 + 2 个新样式表（独立变更，建议单独 commit）

### 长期 (Phase 3)
9. **MCP 支持** — 引入 Model Context Protocol，接入外部工具源
10. **LiteLLM 多提供商统一** — `litellmcreds` 规范与引导流程
