...
│                    cmd/gline/main.go                             │
│                    无参数 → TUI | --gui → GUI | 子命令 → CLI     │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌───────────────────────┐     ┌──────────────────────────┐   │
│  │  TUI 模式 (Bubbletea) │     │  GUI 模式 (Wails v3)     │   │
│  │  internal/tui/*       │     │  internal/gui/*          │   │
│  │  ┌──────┐ ┌────────┐  │     │  ┌─────────┐ ┌────────┐  │   │
│  │  │ chat │ │ input  │  │     │  │Frontend │ │Services│  │   │
│  │  │ view │ │ area   │  │     │  │(Webview)│ │        │  │   │
│  │  ├──────┤ ├────────┤  │     │  └────┬────┘ └───┬────┘  │   │
│  │  │side- │ │ status │  │     │       └─────┬─────┘       │   │
│  │  │ bar  │ │  bar   │  │     │             │             │   │
...
│  │  ┌──────────┐  ┌──────────────┐  ┌──────────────────┐ │    │
│  │  │   LLM    │  │    MCP       │  │     Memory       │ │    │
│  │  │ Providers│  │   Client     │  │  (facts/wiki/rag)│ │    │
│  │  └──────────┘  └──────────────┘  └──────────────────┘ │    │
│  └────────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────────┘

入口路由:
  gline           → TUI (Bubbletea, 默认)
  gline --gui     → GUI (Wails v3)
  gline chat      → CLI 单次对话
  gline history   → CLI 查看历史
  gline kb/wiki/mem → CLI 子命令
```
