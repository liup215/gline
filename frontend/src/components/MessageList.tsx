...
export function MessageList({ messages, showSelectDir, onSelectProjectDir }: MessageListProps) {
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '20px 0', display: 'flex', flexDirection: 'column' }}>
      {messages.length === 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, color: THEME.textDim, gap: '18px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px' }}>
            <h2 style={{ margin: '0 0 8px', fontWeight: 400, fontSize: '1.5rem', color: THEME.textMuted }}>Welcome to gline</h2>
