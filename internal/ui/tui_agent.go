...
// Atomically replace the current agent context/cancel with the new one.
// If an old cancel exists, call it to ensure the previous run is stopped.
m.cancelLock.Lock()
...
// Clean up the stored context/cancel if it's still this run's context.
m.cancelLock.Lock()
if m.agentCtx == ctx {
m.agentCtx = nil
m.agentCancel = nil
