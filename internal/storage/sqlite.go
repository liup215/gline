...
func (s *SQLiteStore) SaveMessage(taskID string, msg types.Message) error {
	...
	_, err := s.db.Exec(`
		INSERT INTO messages (task_id, role, content, reasoning_content, tool_calls, tool_call_id, available_tools, tool_choice, created_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, taskID, string(msg.Role), msg.Content, msg.ReasoningContent, string(toolCallsJSON), msg.ToolCallID, string(msg.AvailableTools), msg.ToolChoice, msg.Timestamp)
	if err != nil {
...
func (s *SQLiteStore) GetMessagesPaginated(taskID string, limit, offset int) ([]MessageRecord, error) {
	...
	for rows.Next() {
		...
		var toolChoice sql.NullString
		if err := rows.Scan(&m.ID, &m.TaskID, &m.Role, &m.Content, &reasoningContent, &toolCalls, &toolCallID, &availableTools, &toolChoice, &m.CreatedAt); err != nil {
		...
		m.ToolChoice = toolChoice.String
